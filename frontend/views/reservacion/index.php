...
$terminadas = Reservacion::find()
        ->where(['estado' => 2])
        ->andWhere(['>=', 'fecha_entrada', $fe_terminada])
...
//print_r($activas);die;
                            ...
                            for ($i = 0; $i < count($activas); $i++) {
                                ...
                                $fe_en = explode('-', $activas[$i]->fecha_entrada);
                                $fecha_ent = $fe_en[2] . '-' . $fe_en[1] . '-' . $fe_en[0];

                                $fe_sa = explode('-', $activas[$i]->fecha_salida);
                    ...
                    <table  class="table table-striped table-bordered" id="denegadas">
                        ...
                        <tbody>
                            ...
                            for ($i = 0; $i < count($denegadas); $i++) {
                                ...
                                //print_r($denegadas[$i]->fecha_entrada);die;
                                
                                if($denegadas[$i]->fecha_entrada <= date("Y-m-d")){
                                    $denegadas[$i]->delete();
                                }
